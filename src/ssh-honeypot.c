...
/* This reads the content of a text file: https://stackoverflow.com/a/3464656 */
char* read_bee_movie_script(char *filename) {
   char *buffer = NULL;
...
int main (int argc, char *argv[]) {
  ...
  while ((opt = getopt (argc, argv, "vh?p:dLl:a:b:i:r:f:su:j:J:P:")) != -1) {
    ...
    case 'b': /* Specify banner string */
      if (optarg == 'bee-movie') { /* Shoutout to Nithin for this idea */
        banner = read_bee_movie_script('assets/entire_bee_movie_script.txt');
      }
      else {
        banner = optarg;
      }
