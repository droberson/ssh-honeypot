...
# ssh-honeypot-unique-ips-password.sh -- by Daniel Roberson
# -- prints a list of unique IP addresses which have tried to authenticate
# -- to ssh-honeypot with a password.
#
# -- usage: ssh-honeypot-unique-ips-password.sh <logfile>
...
grep -v -e "Error exchanging keys" $1 -e "] FATAL" -e "] ssh-honeypot " | awk {'print $6'} |sort |uniq
